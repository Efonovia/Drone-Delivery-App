...
import { formatDate, isDateGreaterOrEqual } from '../utils/utils';
...
function TrackDelivery() {
    ...
    const { pickupLocation, deliveryLocation, deliveryScheduledDate } = getDeliveryDetailsQuery.data?.body

    // if(isDateGreaterOrEqual(new Date(deliveryScheduledDate), new Date())) {
    //     return <h2 style={{margin: "200px 200px"}}>It isn't time for your delivery yet. it's on {formatDate(deliveryScheduledDate)}</h2>
    // }

    return (
        <div className='track-body'>
            <div className='za-details'>
                <h1 style={{color: "#fbb11f"}}><GpsFixedIcon sx={{width:"50px", height:"50px", color: "#373063"}} /> Track Your Delivery</h1>
                <>
